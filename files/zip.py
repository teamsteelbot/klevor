...
def zip_nested_folder(zipf, input_base_path:str, input_folder_path:str, ignore_dirs:list[str]=None, ignore_filenames:list[str]=None):
    for root, dirs, filenames in os.walk(input_folder_path):
        # Skip directories in the ignore list
        if ignore_dirs is not None:
            dirs[:] = [d for d in dirs if os.path.relpath(os.path.join(root, d), input_base_path) not in ignore_dirs]

        # Skip files in the directories in the ignore list
        if ignore_dirs is not None:
