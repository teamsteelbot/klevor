...
from albumentations import (
    Compose, RandomBrightnessContrast, HorizontalFlip, ShiftScaleRotate, RandomCrop, BboxParams
)
...
def augment_image(input_to_process_image_path: str | os.PathLike[str], input_to_process_annotations_path: str | os.PathLike[str],
    ...
    transform = Compose([
        ...
        RandomCrop(width=int(image.shape[1] * 0.9), height=int(image.shape[0] * 0.9), p=0.3),
...
def augment_dataset(input_to_process_dir: str | os.PathLike[str], output_augmented_dir: str | os.PathLike[str],
    ...
    input_to_process_images_dir = os.path.join(input_to_process_dir, DATASET_IMAGES)
    input_to_process_annotations_dir = os.path.join(input_to_process_dir, DATASET_LABELS)

    # Get the output directories
    output_augmented_images_dir = os.path.join(output_augmented_dir, DATASET_IMAGES)
    output_augmented_annotations_dir = os.path.join(output_augmented_dir, DATASET_LABELS)
    output_processed_images_dir = os.path.join(output_processed_dir, DATASET_IMAGES)
    output_processed_annotations_dir = os.path.join(output_processed_dir, DATASET_LABELS)
    ...
    for image_filename in image_filenames:
        ...
        if os.path.exists(input_to_process_annotations_dir):
            augment_image(input_to_process_image_path, input_to_process_annotations_path, output_augmented_images_dir,
...
if __name__ == '__main__':
    ...
    labeled_processed_dir = Files.get_dataset_model_dir_path(DATASET_LABELED, DATASET_PROCESSED,
                                                             arg_yolo_input_model)
    augmented_dir = Files.get_dataset_model_dir_path(DATASET_AUGMENTED, None, arg_yolo_input_model)

    # Augment the dataset
    augment_dataset(labeled_to_process_dir, augmented_dir, NUM_AUGMENTATIONS, labeled_processed_dir)
