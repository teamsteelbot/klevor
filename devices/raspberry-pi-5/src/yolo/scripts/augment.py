...
from ..files.constants import (
    DATASET_IMAGES, DATASET_LABELS, DATASET_LABELED, DATASET_TO_PROCESS,
    DATASET_PROCESSED,
    DATASET_AUGMENTED
)
...
def augment_image(input_to_process_image_path: str | os.PathLike[str],
                  ...
                  output_processed_images_dir: Optional[
                      str | os.PathLike[str]] = None,
                  output_processed_annotations_dir: Optional[
                      str | os.PathLike[str]] = None,
    ...
    transform = Compose([
        ...
        ShiftScaleRotate(shift_limit=0.2, scale_limit=0.2, rotate_limit=25,
                         p=0.5),
        ...
        RandomCrop(width=int(image.shape[1] * 0.9),
    ...
    try:
        for i in range(num_augmentations):
            ...
            if output_processed_images_dir:
                Files.move_file(input_to_process_image_path,
...
def augment_dataset(input_to_process_dir: str | os.PathLike[str],
    ...
    input_to_process_images_dir = os.path.join(input_to_process_dir,
                                               DATASET_IMAGES)
    input_to_process_annotations_dir = os.path.join(input_to_process_dir,
                                                    DATASET_LABELS)

    # Get the output directories
    output_augmented_images_dir = os.path.join(output_augmented_dir,
                                               DATASET_IMAGES)
    output_augmented_annotations_dir = os.path.join(output_augmented_dir,
                                                    DATASET_LABELS)
    output_processed_images_dir = os.path.join(output_processed_dir,
                                               DATASET_IMAGES)
    output_processed_annotations_dir = os.path.join(output_processed_dir,
                                                    DATASET_LABELS)
    ...
    for image_filename in image_filenames:
        ...
        input_to_process_image_path = os.path.join(input_to_process_images_dir,
                                                   image_filename)
        ...
        if os.path.exists(input_to_process_annotations_dir):
            augment_image(input_to_process_image_path,
...
if __name__ == '__main__':
    ...
    labeled_to_process_dir = Files.get_dataset_model_dir_path(DATASET_LABELED,
                                                              DATASET_TO_PROCESS,
                                                              arg_yolo_input_model)
    labeled_processed_dir = Files.get_dataset_model_dir_path(DATASET_LABELED,
                                                             DATASET_PROCESSED,
                                                             arg_yolo_input_model)
    augmented_dir = Files.get_dataset_model_dir_path(DATASET_AUGMENTED, None,
                                                     arg_yolo_input_model)

    # Augment the dataset
    augment_dataset(labeled_to_process_dir, augmented_dir, NUM_AUGMENTATIONS,
