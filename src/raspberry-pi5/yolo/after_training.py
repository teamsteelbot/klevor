...
from yolo.files import (get_dataset_model_dir_path, get_hailo_hef_dir_path, get_model_best_onnx_path,
                        get_model_hailo_hef_path)


# Remove the YOLO validation folders from the dataset, move the training folder and copy the best ONNX weights to the Hailo HEF folder
def after_training(input_dir, hailo_hef_dir, model_hailo_hef_dir, best_onnx_weights_path):
    ...
    # Move the training folder to the Hailo HEF folder
    input_training_path = os.path.join(input_dir, YOLO_DATASET_TRAINING)
    output_training_path = os.path.join(hailo_hef_dir, YOLO_DATASET_TRAINING)
    ...
    # Copy the best ONNX weights to the model Hailo HEF folder
    if os.path.exists(best_onnx_weights_path):
        shutil.copy(best_onnx_weights_path, model_hailo_hef_dir)
        print(f'Copied {best_onnx_weights_path} to {model_hailo_hef_dir} folder')
    else:
...
if __name__ == '__main__':
    ...
    # Get the Hailo HEF path
    hailo_hef_dir = get_hailo_hef_dir_path()

    # Get the model Hailo HEF path
    model_hailo_hef_dir = get_model_hailo_hef_path(arg_yolo_input_model, arg_yolo_version)

    # Get the best weights path
    best_onnx_weights_path = get_model_best_onnx_path(arg_yolo_input_model, arg_yolo_version)

    # Move the folders
    after_training(organized_dir, hailo_hef_dir, model_hailo_hef_dir, best_onnx_weights_path)
