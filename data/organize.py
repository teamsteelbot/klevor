...
def split_data(input_to_process_dir: str, output_organized_dir: str, output_processed_dir:str=None, train_ratio=0.7, val_ratio=0.2):
    ...
    output_training_dir=os.path.join(output_organized_dir, TRAINING)
    output_training_images_dir=os.path.join(output_training_dir, const.IMAGES)
    output_training_annotations_dir=os.path.join(output_training_dir, const.LABELS)
    output_validations_dir=os.path.join(output_organized_dir, VALIDATIONS)
    output_validations_images_dir=os.path.join(output_validations_dir, const.IMAGES)
    output_validations_annotations_dir=os.path.join(output_validations_dir, const.LABELS)
    output_testing_dir=os.path.join(output_organized_dir, TESTING)
    output_testing_images_dir=os.path.join(output_testing_dir, const.IMAGES)
    output_testing_annotations_dir=os.path.join(output_testing_dir, const.LABELS)

    # Check if the path exists, if not it creates it
    for io_dir in [input_to_process_dir, input_to_process_images_dir, input_to_process_annotations_dir, output_processed_dir, output_processed_images_dir, output_processed_annotations_dir, output_training_dir, output_validations_dir, output_testing_dir,
                     output_training_images_dir, output_training_annotations_dir, output_validations_images_dir, output_validations_annotations_dir, output_testing_images_dir, output_testing_annotations_dir]:
    ...
    for i, image_filename in enumerate(image_filenames):
        ...
        if output_processed_dir is not None:
            ...
            files.move_file(input_to_process_annotations_path, os.path.join(output_processed_annotations_dir, annotations_filename))
