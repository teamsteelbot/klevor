...
1. **Entrenamiento de forma local**: Para ello, se debe contar con una GPU dedicada para el entrenamiento.
	1. En este caso, debemos ejecutar el script `yolo/scripts/train.py` para entrenar el modelo YOLOv11. Este script utiliza la librería [Ultralytics](../libraries/python.es.md#ultralytics-yolo) para realizar el entrenamiento del modelo y guardar los pesos en la carpeta `yolo/v11/runs/g`.
