...
The main purpose of this library is to receive all of the BNO08X sensor's data and to simplify the way the users manage its data, like for example, simplify the calculations of quaternions (similar to a 3D vector), to be able to manage its relative position.

As for Klevor, we used this library along the [gyroscope](../../electronic/components/current.es.md#gyroscope-gy-bno085), the main purpose is to, whenever Klevor is turning in the middle of the game field, it can easily determine its yaw angle and, to determine when it has turned exactly 90 degrees and continue with the Obstacle Detection.
